    ...
    dispatch_once(&onceToken, ^{
        ...
        for (size_t i = 0; i < sizeof(candidates) / sizeof(candidates[0]); i++) {
            if (dlopen(candidates[i], RTLD_LAZY | RTLD_GLOBAL)) {
                NSLog(@"[ExternalNoelx] ls: loaded %s", candidates[i]);
                return;
            }
        }
        NSLog(@"[ExternalNoelx] ls: CoreServices/MobileCoreServices dlopen failed");
    });
}
...
static NSDictionary *appsFromWorkspace(void) {
    ...
    for (NSString *selectorName in @[@"allApplications", @"allInstalledApplications"]) {
        ...
        NSLog(@"[ExternalNoelx] ls: %@ returned %lu", selectorName,
              (unsigned long)([candidate isKindOfClass:[NSArray class]] ? [candidate count] : 0));
    }
    if (!apps || apps.count == 0) {
        NSLog(@"[ExternalNoelx] ls: workspace enumeration empty");
        return result;
    }
    NSLog(@"[ExternalNoelx] ls: %@ returned %lu proxies", usedSelector, (unsigned long)apps.count);
    ...
    NSLog(@"[ExternalNoelx] ls: extracted %lu apps (%lu with container)",
...
NSDictionary<NSString *, NSDictionary *> *installedAppInfo(void) {
    ...
    NSLog(@"[ExternalNoelx] ls: workspace empty; MobileInstallation=%lu",
